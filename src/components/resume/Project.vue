        ...
        data: [
	        {
		        ...
		        workDescs: [
			        '主要工作负责该项目的维护及迭代开发工作',
			        '实现每一排的座位需要根据设定的规则完成排序计算，规则分为从左到右，从右到左，中左右和中右左四种',
              '实现画布选转180°，旋转后画布的移动方向、内容显示错误及原有的拖拽操作出现错误，所以重写画布移动时边界规则计算、每个具体元素做旋转及' +
              '处理拖拽时坐标的反向计算推出正确坐标',
              '座位在满足条件时，可以左右移动，实现座位的错位排列效果，如果遇到左右边界或过道时不移动，实现该功能主要时先计算移动的整体涉及到哪些座位，根据' +
              '该整体计算出移动的边界，判断是否在边界坐标中，若在则不允许移动；同时拖拽新增的座位及过道时也要考虑到是否和错位的元素重合及一系列细节问题的处理',
              '区域选择座位，实现框选覆盖元素后选中颜色，框选移动中若未覆盖则还原成原来颜色，主要通过鼠标事件实现一个选择框，框选时计算座位是否在框中，若在则表示选中，' +
              '正常情况下，若元素的原坐标不在框中，但和覆盖框重合，也应该选中，所以在框选的原坐标应该减少座位元素的宽高，然后实现框选功能'
		        ],
		        technicals: [
			        'vue、vuex、vue-router、element-ui、axios、stylus、webpack、konva.js、typescript'
		        ]
	        },
          {
